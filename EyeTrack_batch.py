...
# -*- coding: utf-8 -*-
"""
EyeTrack.py (batch-safe rewrite)

Key fixes:
- Correctly uses MI_MNIST fields: image / mi_map / label
- Ensures images are shaped (B, 1, H, W) for conv nets
- Removes accidental reassignments that break device placement
- Removes extra unsqueeze() that would create (1, B, 1, 28, 28)
- Uses batch-safe classifier outputs (no Softmax before CrossEntropyLoss)
"""
...
for epoch in range(epochs):
    for i, batch_data in enumerate(train_loader):
        ...
        loss_alpha = loss_c * loss_search.detach()
